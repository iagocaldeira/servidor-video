...
public class Servidor extends Model {
    ...
    public double tempoSimulacao = 5;
    public TimeUnit unidadeSimulacao = TimeUnit.MINUTES;

    /**
     * filaClientes: variável responsável por armazenar todos os clientes que estão
     * aguardando a máquina de lavar ser liberada para utilizarem-na.
     */
    ...
    /**
     * distribuicaoTempoChegadasClientes: distribuição do tempo entre chegadas
     * sucessivas de clientes à lavanderia. Será usada uma distribuição exponencial,
     * com média de 40 minutos.
     */
    ...
    /**
     * distribuicaoTempoServicoMaquinaLavar: distribuição do tempo de serviço da
     * máquina de lavar, ou seja, do tempo gasto pela máquina de lavar para servir
     * (lavar as roupas) os clientes da lavanderia. Será usada uma distribuição
     * uniforme, com valores entre 20 e 40 minutos.
     */
     ...
     *
     * Cria um novo modelo, que representa o modelo de eventos discretos da
     * lavanderia. A criação do modelo da lavanderia ocorre por meio da chamada do
     * método construtor da super-classe, ou seja, da classe Model.
     * 
     * parâmetro owner: indica o modelo do qual esse modelo é parte. Deve ser setado
     * para null quando não existir um modelo, do qual esse novo modelo criado faz
     * parte.
     * 
     ...
     *
     * parâmetro showInTrace: é um flag que indica se o modelo deve ou não produzir
     * saídas para um trace de saída.
     */
     ...
     * Método description
     * 
     * Retorna uma string que descreve o modelo criado. Essa descrição é
     * automaticamente incluída no relatório da simulação que é criado.
     */
    public String description() {
        return ("Esse é o modelo de eventos discretos de uma lavanderia. "
                ...
                + "para ser utilizada. " + "Depois que a lavagem das roupas trazidas pelo cliente termina, "
    ...
    public void init() {
      ...
      Dispositivo dDisco2 = new Dispositivo(this, "Disco 2", true, 3);
         ...
         * Nesse caso, como o flag foi setado para "true", a distribuição deverá retornar apenas valores positivos.
         */
        distribuicaoTempoChegadasClientes.sampleTimeSpan(TimeUnit.MILLISECONDS);
        distribuicaoTempoChegadasClientesPool.sampleTimeSpan(TimeUnit.MILLISECONDS);
        distribuicaoTempoProcessador1.sampleTimeSpan(TimeUnit.MILLISECONDS);
        distribuicaoTempoProcessador2.sampleTimeSpan(TimeUnit.MILLISECONDS);
        distribuicaoTempoDiscoRapido.sampleTimeSpan(TimeUnit.MILLISECONDS);
        distribuicaoTempoDiscoLento.sampleTimeSpan(TimeUnit.MILLISECONDS);
    }
     ...
     * 
     * Esse método é utilizado para escalonar todos os eventos, na lista interna de
     * eventos do simulador, necessários para iniciar a simulação.
     * 
     * Nesse caso, o evento externo gerador de clientes que chegam à lavanderia para
     * lavarem suas roupas é criado e escalonado para ocorrer logo no início da
     * simulação.
     */
    ...
    public void gerarPrimeiroCliente() {
        ...
        eventoGeradorCliente = new EventoGeradorCliente(this,
                "Evento externo responsável por gerar um cliente que chega à lavanderia", true);

        /**
         * Escalona o evento externo "eventoGeradorCliente" para ocorrer em um ponto
         * específico da simulação. Nesse caso, o evento externo deverá ocorrer logo no
         * início da simulação.
         */
        eventoGeradorCliente.schedule(new TimeSpan(0.0, TimeUnit.MILLISECONDS));
    }
    ...
    public void gerarClientes(boolean isPool) {
        ...
        eventoGeradorCliente.schedule(new TimeSpan(instanteChegadaCliente,TimeUnit.MILLISECONDS));
    }

    /**
     * Método responsável por retornar uma amostra da distribuição de probabilidade
     * utilizada para determinar o momento de chegada, na lavanderia, do próximo
     * cliente.
     */
    ...
    /**
     * Método responsável por retornar uma amostra da distribuição de probabilidade
     * utilizada para determinar o tempo de serviço da máquina de lavar-roupas.
     */
     ...
     * novo cliente (passado como parâmetro) que chegou à lavanderia ou ocupada. No
     * primeiro caso, a máquina de lavar roupas é alocada para esse cliente. No
     * segundo caso, esse cliente entra em uma fila de espera, onde irá aguardar a
     * liberação da máquina de lavar roupas.
     * 
    ...
    public void servirCliente(Cliente cliente, int listIndex) {
        ...
        /**
         * Verifica se a máquina de lavar roupas está livre, o que indica que o novo
         * cliente que chegou à lavanderia pode utilizá-la.
         */
        ...
        if ((localDispo.getOcupada()) == false) {
            ...
            // Modifica o estado da máquina de lavar-roupas indicando que, a partir desse
            // momento, ela está sendo utilizada por um cliente.
            localDispo.setOcupada(true);

            // Utilização da máquina de lavar roupas pelo novo cliente que chegou à
            // lavanderia.
            localDispo.lavar(cliente, listIndex);
        } else {
            /**
             * Caso a máquina de lavar roupas esteja ocupada, lavando as roupas trazidas por
             * outro cliente da lavanderia, o novo cliente que chegou à lavanderia entra em
             * uma fila de espera para utilizar a máquina de lavar.
             */
    ...
    /**
     * Método responsável por liberar a máquina de lavar-roupas passada como
     * parâmetro. Caso exista algum cliente aguardando, na fila de espera, para
     * utilizar a máquina de lavar, essa é realocada ao primeiro cliente da fila de
     * espera.
     */
    public void liberarDispositivo(Dispositivo dispositivo, int indexDoRecurso, boolean liberar) {
        ...
        case 0:
            contadorVisitacaoCpu1++;
            break;
        case 1:
            contadorVisitacaoCpu2++;
            break;
        case 2:
            contadorVisitacaoDRapido++;
            break;
        case 3:
            contadorVisitacaoDLento++;
            break;
        default:
            break;
        }
        ...
        // Verifica se existe algum cliente aguardando na fila de espera para utilizar a
        // máquina de lavar.
        if (conjuntoDeFilas.get(indexDoRecurso).isEmpty()) {

            /**
             * Caso não exista nenhum cliente aguardando para utilizar a máquina de lavar
             * roupas, essa é liberada.
             */
            ...
            // O primeiro cliente da fila de espera para utilizar a máquina de lavar é
            // retirado dessa fila.
            cliente = (Cliente) conjuntoDeFilas.get(indexDoRecurso).first();
            conjuntoDeFilas.remove(cliente);

            // Utilização da máquina de lavar-roupas pelo primeiro cliente da fila de
            // espera.
        ...
        if (liberar) {
            ...
            switch (indexDoRecurso) {
            case 0:
                contadorLiberacaoCpu1++;
                break;
            case 1:
                contadorLiberacaoCpu2++;
                break;
            case 2:
                contadorLiberacaoDRapido++;
                break;
            case 3:
                contadorLiberacaoDLento++;
                break;
            default:
                break;
            }
    ...
    public static void main(String[] args) {
        ...
        String questao1 = "", questao2 = "", questao3 = "", questao4 = "", questao5 = "", questao6 = "", questao7a = "",
                questao7b = "", questao7c = "";
        ...
        for (int i = 0; i <= 100; i++) {
            ...
            experimento.tracePeriod(new TimeInstant(0),
                    new TimeInstant(modeloServidor.tempoSimulacao, modeloServidor.unidadeSimulacao));
            experimento.start();
            experimento.report();
            experimento.finish();
            ...
            tempoProcessador1 = (modeloServidor.totalTempoDistribuicaoProcessador1
                    / modeloServidor.qtdTempoDistribuicaoProcessador1);
            tempoProcessador2 = (modeloServidor.totalTempoDistribuicaoProcessador2
                    / modeloServidor.qtdTempoDistribuicaoProcessador2);
            tempoDiscoRapido = (modeloServidor.totalTempoDistribuicaoDiscoRapido
                    / modeloServidor.qtdTempoDistribuicaoDiscoRapido);
            tempoDiscoLento = (modeloServidor.totalTempoDistribuicaoDiscoLento
                    / modeloServidor.qtdTempoDistribuicaoDiscoLento);
            ...
            System.out.println("--------");
            // System.out.println(modeloServidor.tempoTotalResposta/
            // modeloServidor.contadorVisitacao);
            System.out.println("--------");
            ...
            // questao6 += i + ";" + utilizacaoD1 + ";" + utilizacaoD2 + ";" + utilizacaoD3
            // + ";" + utilizacaoD4 + ";\n";
        }
        ...
        EscreveArquivo.preparaArquivo(questao1, questao2, questao3, questao4, questao5, questao6, questao7a, questao7b,
                questao7c);
    }
